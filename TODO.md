...
## Low Priority
- [x] ~~Migrate GitHub Pages to `sdk-example.aitc.dev` custom domain~~ — done. `public/CNAME` lands at site root, `BASE_PATH` env removed from deploy workflow, Cloudflare CNAME `sdk-example → apps-in-toss-community.github.io` added, GitHub Pages custom domain set + HTTPS enforced. Homepage's `content/projects.ts` `demoUrl` flip happens in step 4 of the staged cutover (separate PR on the homepage repo).
