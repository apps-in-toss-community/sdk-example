...
| `pnpm build` | OG 이미지 생성 + 타입 체크 + 프로덕션 빌드 + 라우트별 HTML 생성 → `dist/` |
...
- **이미지 빌드**: `pnpm build:og` 또는 `pnpm build`의 첫 단계에서 `scripts/build-og-images.tsx`가 `public/og/<slug>.png` 19장(home + 18 그룹)을 생성한다. PNG는 commit 대상.
...
- 컴포넌트 카피 추출은 공용 컴포넌트(`ApiCard`, `HistoryLog`, `ResultView`, `PageHeader`, `ErrorBoundary`, `DemoBanner`, `PolyfillNotice`, `PolyfillToggleCard`)와 `HomePage`까지 완료. 도메인 페이지(`AuthPage`, `IAPPage` 등)의 ApiCard `description`/`label`/`placeholder` props 추출은 follow-up.
