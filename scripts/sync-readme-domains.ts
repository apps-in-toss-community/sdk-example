...
function extractStringField(objSrc: string, field: string): string | null {
  // Lookbehind blocks suffix matches: searching `path` must not match `subpath`.
  const re = new RegExp(`(?<![A-Za-z0-9_])${field}\\s*:\\s*(['"\`])((?:\\\\.|(?!\\1).)*)\\1`);
...
// Normalize CRLF→LF so a Windows checkout (autocrlf) doesn't false-positive
// against the LF-only output of renderTable().
...
function main(): void {
  ...
  const readmeSrc = normalizeNewlines(readFileSync(README_MD, 'utf8'));
