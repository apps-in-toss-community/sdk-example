 ...
 * `aitcc app ads placement-groups create`(2026-07-24)로 발급해 콘솔에 등록한 값이다.
 * 다만 **실기기 광고 송출은 아직 확인되지 않았다** — 실기기(31146)에서 이 값으로 load를
 * 호출하면 테스트 ID(`ait-ad-test-*`)와 동일하게 `PLACEMENT_ID_FETCH_FAILED`가 발생한다
 * (2026-07-25/26 관측). 클라이언트 코드에 넣는 공개 식별자라 비밀은 아니지만 **이 예제
 * 앱 전용**이다 — 이 boilerplate를 복사해 다른 미니앱을 만든다면 반드시 본인 콘솔에서
 * 발급받은 지면 ID로 교체해야 한다(#355). AdsPage의 "실 지면(dog-food)" 프리셋이 이 값을
 * 소비한다.
 *
