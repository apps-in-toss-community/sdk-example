 ...
 * 프로비저닝된 앱"(로그인 연동됨, 광고 서빙됨, 알림 템플릿 등록됨, 권한 선언됨)을
 * 모델링해 낙관적으로 resolve하는데, 실기기 31146에서는 같은 호출이 네이티브
 * 오류로 reject되는 것이 관측됐다. 그래서 같은 호출이 env1은 resolve, env3는
 * reject한다.
 *
...
const PROVISIONING_31146 = {
  ...
  /** `ads` — `happy-load`: 실기기 load가 이 코드로 거부되는 것만 관측됨(원인 미확정, #358). */
 ...
 *
 * 실기기 31146에서 아래 API는 reject가 아니라 "성공했지만 내용이 빈/오류 본문"
 * shape로 resolve하는 것이 관측됐다 (env3 run11 2.x/iOS 실측). 원인은 단정하지
 * 않는다 — 콘솔 프로비저닝 축이 여럿이라 현재 증거로 가르지 못했다(#298):
