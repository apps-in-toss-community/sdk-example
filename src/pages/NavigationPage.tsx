    ...
    <div>
      ...
      <div className="p-4 space-y-3">
        ...
        <ApiCard
          ...
          execute={async (p) => { openURL(p.url); }}
        />
        <ApiCard
          name="share"
          description="메시지 공유"
          params={[{ name: 'message', label: 'Message', placeholder: '공유할 메시지' }]}
          execute={async (p) => { await share({ message: p.message }); }}
        />
        <ApiCard
        ...
        <ApiCard
          ...
          execute={async (p) => { setIosSwipeGestureEnabled({ isEnabled: p.isEnabled }); }}
        />
        <ApiCard
          ...
          execute={async (p) => { setDeviceOrientation({ type: p.type }); }}
        />
        <ApiCard
